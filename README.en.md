...
> Generated files carry a protocol version constant `OCTL_PROTOCOL_VERSION` matching the `octl` binary. On startup the daemon automatically writes protocol-matching plugins into `~/.config/opencode/plugins/` (skipped when the content already matches); on a version mismatch the daemon rejects the subscription and repairs the files the same way. Note that opencode does **not** hot-reload TUI plugins (verified on 1.18.30) — a rejected sidebar stops reconnecting and shows a "⟳ Restart" button; clicking it restarts the instance in place to load the new plugin (tmux respawn-pane when attached, TIOCSTI-injected restore command in bare terminals). Upgrading octl is still **replace the binary and restart the daemon**; newly started instances pick up the new plugins automatically.
