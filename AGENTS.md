...
- `statusbar.js` 是纯逻辑层（groupCounts 定序非零计数 / groupTooltip 分组折叠 / collectFavorites+favoriteRows 收藏列表 / classifyJump 四分类 / buildJumpScript 四场景 sh 脚本），**QML import 与 bun CJS 互操作双兼容**（QML JS 资源不支持 export，靠 `typeof module` 守卫导出；测试经默认导入解构）。仅 `statusbar.qml` 注入 `{{OCTL_MD5}}`（`RenderedOmarchyWidgetFiles`）；协议 MD5 仍只覆盖 opencode 双插件，本 widget 不影响既有升级语义。
- widget 订阅 daemon `view` 频道：全 IDLE/未连接自隐藏，断连指数退避 1s→30s；弹层为 `Ui.PopupCard` 默认 **click 触发**（HyprlandFocusGrab 点外关闭，与 Tray 同款生产路径）——**hover 触发两条路线真机均闪烁循环，不可用**：KeyboardPanel（全屏 layer-shell + 整屏 dismissArea，打开即截走 bar 的 hover）、PopupCard `triggerMode:"hover"`（亦复现）；弹层内容为「状态分组列表（标题截断、超 8 条折叠 `+N more`）+ 收藏列表（`★ Favorites` 标题黄色 `#e0af68` 与 sidebar 收藏高亮同色，读 daemon 推送的 `ViewMsg.favorites` 原序全量展示、不折叠，空则整节隐藏；行不带状态图标，纯逻辑层 `collectFavorites`/`favoriteRows`）」；行点击跳转在本地执行（attached：client tty 探测 + hyprctl focuswindow + switch-client/select-window/select-pane；detached：`omarchy launch terminal tmux attach`；bare：pid PPID 上溯 focuswindow，上溯穷尽无窗口（headless 实例：`opencode serve` / daemon 后台 run，祖先链在 supervisord/systemd 断掉）回落 dead 行为拉起查看实例；dead：`makeTmuxSessionName` + `new-session` + TUI `opencode --session`，与 bare 回落共用 `deadTail`）——收藏行与状态行共用同一 `buildJumpScript` 入口。所有 tmux 目标单引号包裹。focuswindow 为**双语法 fallback**（`focuswindow pid:N || hl.dsp.focus({window="pid:N"})`）：Hyprland 0.56 起 Lua 配置管理器下 `hyprctl dispatch` 被原样拼进 `hl.dispatch(<in>)` 的 Lua 桥执行（v0.56.2 `HyprCtl.cpp dispatchRequest`），旧 `focuswindow pid:N` 拼出非法 Lua 必然报错（症状即"tmux 切了但人不跳工作区"），`hl.dsp.focus` 是 Lua 配置下的正确形态（走的仍是带切工作区语义的 `Actions::focus`）；先试旧语法保 hyprlang 配置兼容，失败回退 Lua 表达式。
